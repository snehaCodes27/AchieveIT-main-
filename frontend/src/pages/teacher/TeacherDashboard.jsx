...
const PROFESSOR_CATEGORIES = [
  ...
  {
    id: "courses_certifications",
    title: "Courses & Online Certifications",
    iconType: "course-orange",
    bgColor: "#ffedd5",
    iconColor: "#ea580c",
  },
  {
    id: "paper_publications",
  ...
  {
    id: "presentations_talks",
    title: "Presentations & Invited Talks",
    iconType: "presentation-green",
    bgColor: "#dcfce7",
    iconColor: "#16a34a",
  },
  {
...
export default function TeacherDashboard() {
  ...
  const rawFacultyId = user?.college_id || user?.collegeId || user?.faculty_id || "2024DITP01";
  const [profileImage, setProfileImage] = useState(
    localStorage.getItem(`prof_avatar_${rawFacultyId}`) || ""
  );
  ...
  const fetchAchievements = async () => {
    try {
      ...
      console.error("Failed to load professor achievements:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const matchCategory = (itemCategory, targetId) => {
    ...
    const c = itemCategory.toLowerCase().replace(/[^a-z0-9]/g, "");
    const target = targetId.toLowerCase().replace(/[^a-z0-9]/g, "");
    if (c === target) return true;
    if (target === "fdp" && (c.includes("fdp") || c.includes("facultydev"))) return true;
    if (target === "coursescertifications" && (c.includes("course") || c.includes("certif") || c.includes("online"))) return true;
    if (target === "paperpublications" && (c.includes("paper") || c.includes("publi") || c.includes("journal") || c.includes("research"))) return true;
    if (target === "presentationstalks" && (c.includes("present") || c.includes("talk") || c.includes("keynote") || c.includes("speaker") || c.includes("seminar"))) return true;
    if (target === "otherprofessional" && (c.includes("other") || c.includes("activity") || c.includes("workshop") || c.includes("committee"))) return true;
  ...
  const handleSaveAvatar = (e) => {
    ...
    try {
      localStorage.setItem(`prof_avatar_${rawFacultyId}`, previewAvatar);
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      setProfileError(err.message || "Failed to update password. Please check your current password.");
    }
  };
  ...
  const handleCompressFile = async () => {
    ...
    setFormError("");
    setCompressionProgress("Optimizing document resolution & reducing file size...");
  ...
  const runOcrScan = async (fileToScan) => {
    ...
    setIsScanning(true);
    setScanMessage("🤖 AI is reading certificate details & verifying professor identity...");
    setFormError("");
    setNameValidation(null);
    setIsNameMismatch(false);
    ...
    try {
      ...
      if (scanResult) {
        ...
        if (scanResult.is_duplicate) {
          setFormError(`⚠️ Duplicate Detected: ${scanResult.duplicate_reason || "This document was already uploaded."}`);
        } else if (isMismatch) {
          setFormError(`⚠️ Certificate Owner Mismatch: ${scanResult.mismatch_warning || "Name does not match your faculty account."}`);
        } else {
          setScanMessage("✨ AI Auto-fill complete & faculty identity verified!");
        }

        // Map OCR category or default
        let detectedCat = "fdp";
        const catStr = (d.category || "").toLowerCase();
        if (catStr.includes("course") || catStr.includes("certif")) detectedCat = "courses_certifications";
        else if (catStr.includes("paper") || catStr.includes("publi") || catStr.includes("journal")) detectedCat = "paper_publications";
        else if (catStr.includes("present") || catStr.includes("talk") || catStr.includes("speak") || catStr.includes("keynote")) detectedCat = "presentations_talks";
        else if (catStr.includes("other") || catStr.includes("workshop")) detectedCat = "other_professional";
      ...
      setScanMessage("Document uploaded. Please verify and fill the fields below.");
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData.file && !formData.fileUrl) {
      setFormError("Please upload and scan a valid certificate or proof document.");
      return;
    }
    if (isNameMismatch) {
      setFormError("Cannot submit: Document name does not match your faculty account.");
      return;
    }
    ...
    if (!formData.issuer.trim()) {
      setFormError("Issuing Organization / Publisher is required.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        participant_name: formData.participantName || rawProfName,
        ...
        file_name: formData.fileName || "faculty_proof.pdf",
      ...
      setFormSuccess("🎉 Professional achievement recorded and verified successfully!");
  ...
  const rawProfName = user?.name || user?.fullName || "Priya Deshmukh";
  const displayProfName = rawProfName.toLowerCase().startsWith("prof") || rawProfName.toLowerCase().startsWith("dr")
    ? rawProfName
    : `Prof. ${rawProfName}`;
  const firstName = rawProfName.replace(/^(Prof\.|Dr\.|Mr\.|Mrs\.|Ms\.)\s+/i, "").split(" ")[0] || "Priya";
  const initials = rawProfName
  ...
  return (
    <div className="ach-exact-root">
      ...
      <div className="ach-side-card ach-quote-right">
        <div className="ach-chart-icon-box" style={{ background: "#ede9fe" }}>
      ...
      <header className="ach-header">
        <div className="ach-header-left">
          ...
          <span className="ach-prof-pill">PROFESSOR</span>
          <span style={{ background: "#dcfce7", color: "#15803d", padding: "3px 8px", borderRadius: "12px", fontSize: "11px", fontWeight: "700", marginLeft: "6px" }}>
            🟢 ACTIVE
          </span>
        </div>
        ...
        <div className="ach-header-right">
          ...
          >
            ...
            <div className="ach-user-details">
              <div className="ach-user-fullname">{displayProfName}</div>
              <div className="ach-user-subid">ID: {rawFacultyId}</div>
            </div>
      ...
      <div className="ach-container">
        ...
        <section className="ach-hero-card">
          <div className="ach-hero-left">
            ...
            <h1 className="ach-hero-title">Welcome back, Prof. {firstName}! 👋</h1>
            <p className="ach-hero-subtitle">
              Track, manage, and showcase your professional achievements.
            </p>
            <p className="ach-hero-quote">
              “Continuous learning leads to greater impact.”
            </p>
          </div>

          <div className="ach-hero-right">
            <div className="ach-hero-art">
              <div className="ach-art-circle-bg" style={{ background: "radial-gradient(circle, #f3e8ff 0%, #e0e7ff 100%)" }}></div>
              <div className="ach-art-illustration">
                <svg width="150" height="110" viewBox="0 0 160 120" fill="none">
                  <path d="M25 85 C20 70 30 55 45 60 C40 75 30 85 25 85 Z" fill="#c4b5fd" opacity="0.8" />
                  <path d="M15 95 C10 85 18 75 30 78 C25 88 18 95 15 95 Z" fill="#a78bfa" opacity="0.6" />
                  <rect x="70" y="82" width="60" height="22" rx="4" transform="rotate(-15 70 82)" fill="#ffffff" stroke="#e2e8f0" strokeWidth="2" />
                  <rect x="94" y="74" width="10" height="22" transform="rotate(-15 94 74)" fill="#8b5cf6" />
                  <polygon points="85,35 25,60 85,85 145,60" fill="#4338ca" />
                  <polygon points="85,35 145,60 85,85 25,60" stroke="#3730a3" strokeWidth="1.5" />
                  <path d="M50 72 V88 C50 96 85 102 85 102 C85 102 120 96 120 88 V72" fill="#1e1b4b" />
        ...
        <section className="ach-categories-wrapper">
            ...
            <div className="ach-cat-title-left">
              ...
              <div>
                ...
                <p className="ach-cat-subtext">
                  Explore and manage your professional achievements by category
                </p>
              </div>
            </div>
            <span className="ach-cat-badge">5 Categories</span>
          </div>

          {/* Row 1: 3 Cards */}
          <div className="ach-prof-grid-row-1">
            {PROFESSOR_CATEGORIES.slice(0, 3).map((cat) => {
          ...
          {/* Row 2: 2 Cards */}
          <div className="ach-prof-grid-row-2">
            {PROFESSOR_CATEGORIES.slice(3, 5).map((cat) => {
              const count = getCategoryCount(cat.id);
              return (
                <div
                  key={cat.id}
      ...
      {/* ===================================================
          MODAL 1: PROFILE & PASSWORD SETTINGS
          =================================================== */}
      {showProfileModal && (
          ...
          <div className="std-modal-content ach-profile-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ach-profile-overview-box">
              ...
              <div className="ach-overview-meta">
                <h3>{displayProfName}</h3>
                <span className="ach-overview-id">Faculty ID: {rawFacultyId}</span>
              ...
              <form onSubmit={handleSaveAvatar} className="ach-avatar-form">
                <div className="ach-avatar-upload-zone">
                  ...
                  <div className="ach-avatar-controls">
                    <input
                      type="file"
                      id="profAvatarFileInput"
                      accept="image/*"
                      onChange={handleAvatarChange}
                      style={{ display: "none" }}
                    />
                    <label htmlFor="profAvatarFileInput" className="std-btn-secondary ach-choose-img-btn">
                      📁 Select Image
                    </label>
      ...
      {/* ===================================================
          MODAL 2: SUBMIT NEW FACULTY ACHIEVEMENT
          =================================================== */}
      {showAddModal && (
        <div className="std-modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="std-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <h2>✨ Submit Professional Achievement</h2>
                <p>Upload certificate / proof document and verify all details below</p>
              </div>
            ...
            {isNameMismatch && nameValidation && (
              ...
              >
                ...
                <p style={{ margin: "6px 0 10px 0", fontSize: "13px", color: "#991b1b", lineHeight: "1.4" }}>
                  Name on document does not match your faculty account name. You can only submit achievements belonging to your profile.
                </p>
                <div
                  style={{
                    background: "#ffffff",
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px dashed #f1f5f9", paddingTop: "6px" }}>
                    <span style={{ color: "#64748b", fontWeight: "600" }}>Your Faculty Name:</span>
                    <strong style={{ color: "#16a34a" }}>{nameValidation.user_name || rawProfName}</strong>
                  </div>
                </div>
              </div>
            )}
            ...
            {!isNameMismatch && nameValidation && nameValidation.is_match && (
              ...
              >
                ...
                <span>{nameValidation.message || `Document verified for ${rawProfName}`}</span>
              </div>
            )}
            ...
            <form onSubmit={handleFormSubmit} className="std-modal-form">
              ...
              <div className="std-upload-dropzone">
                <input
                  ...
                  style={{ display: "none" }}
                />
                <label htmlFor="profCertUpload" className="upload-label">
                  <div className="upload-icon-circle">📂</div>
                  <strong>
                    {formData.fileName ? formData.fileName : (selectedRawFile ? selectedRawFile.name : "Upload Certificate / Proof * (Required)")}
                  </strong>
                  <span>
                    {selectedRawFile
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                    <span style={{ fontSize: "12px", color: "#6b7280" }}>
                      Compressing optimizes the document for faster AI scanning without losing text clarity.
                    </span>
                    <button
                      type="button"
              ...
              <div className="std-form-grid">
                ...
                <div className="std-form-group full-width">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. 5-Day Online FDP on Cloud Computing & AI"
                ...
                <div className="std-form-group">
                  ...
                  <input
                    type="text"
                    placeholder="e.g. FDP-2025-08492 / IEEE-PUB-991 (if visible)"
                ...
                {/* 4. Issuing Organization / Institute / Journal (Required) */}
                <div className="std-form-group">
                  <label>Issuing Organization / Institute / Journal *</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. AICTE, ATAL, IEEE, Springer, NPTEL"
                ...
                {/* 6. Description / Remarks (Optional) */}
                <div className="std-form-group full-width">
                  <label>Description / Remarks (Optional)</label>
                  <textarea
                    rows={3}
                    placeholder="Add paper DOI, conference name, key topics, or outcome (optional)..."
            ...
            <div className="modal-header">
              <div className="cat-modal-title-row">
                ...
                <div>
                  ...
                  <p>Filtered faculty records for your portfolio</p>
                </div>
              </div>
            ...
            <div className="records-list-container">
              {categoryRecords.length === 0 ? (
                <div className="no-records-box">
                  ...
                  <h4>No achievements recorded in this category yet</h4>
                  <p>Submit your proof document to get it verified and counted.</p>
                  <button
                    className="std-btn-primary"
                    onClick={() => {
                    ...
                    <tbody>
                      {categoryRecords.map((item, idx) => (
                        <tr key={item.id || item._id || idx}>
                          ...
                          <td>{item.date || item.event_date || item.issueDate || "—"}</td>
                          <td>
                            <span
                          ...
                          <td>
                            {item.certificate_url || item.certificateUrl || item.fileUrl ? (
                              <button
                                className="view-cert-btn"
                                onClick={() =>
                                  setPreviewCertUrl(item.certificate_url || item.certificateUrl || item.fileUrl)
                                }
                              >
                                View File ↗
                              </button>
      ...
      {previewCertUrl && (
        <div className="std-modal-overlay preview-overlay" onClick={() => setPreviewCertUrl(null)}>
          <div className="std-modal-content preview-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <h3>Faculty Proof Document Preview</h3>
